...
read_casco_comp <- function(tbl, data_dir) {
  ...
  read_autoseg(
    file_path,
    n_max = 10, col_types = paste0(rep("c", ncol_casco_comp), collapse = "")
  )
}
```
...
```{r, echo = FALSE}
arq_casco_comp <- read_casco_comp(
...
```{r, echo = FALSE}
arq_casco3_comp <- read_casco_comp(
...
```{r, echo = FALSE}
arq_casco4_comp <- read_casco_comp(
